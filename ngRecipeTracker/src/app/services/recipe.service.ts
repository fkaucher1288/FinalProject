...
export class RecipeService {
  ...
  index(): Observable<Recipe[]> {
    return this.http.get<Recipe[]>(this.url, this.getHttpOptions()).pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError(
          () => new Error('RecipeService.index(): error deleting todo')
        );
      })
    );
  }
    ...
    return this.http
      ...
      .pipe(
        catchError((error: any) => {
          ...
          console.log(error);
          return throwError(
            () => new Error('RecipeService.update(): error updating recipe')
          );
        })
      );
  }
  ...
  destroy(recipeId: number): Observable<void> {
    return this.http
      .delete<void>(this.url + '/' + recipeId, this.getHttpOptions())
      .pipe(
        catchError((error: any) => {
          console.log('RecipeService.destory(): error deleting recipe: ');
          console.log(error);
          return throwError(
            () => new Error('RecipeService.destory(): error deleting recipe')
          );
        })
      );
  }
  ...
  show(recipeId: number): Observable<Recipe> {
    return this.http
      .get<Recipe>(this.url + '/' + recipeId, this.getHttpOptions())
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
            () =>
              new Error('RecipeService.show(): error showing recipe with id')
          );
        })
      );
  }
  ...
  getUserRecipes(userId: number): Observable<Recipe[]> {
    return this.http
      .get<Recipe[]>(this.url + '/userrecipes/' + userId, this.getHttpOptions())
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
  ...
  addRecipeRating(rating: RecipeRating) {

     return this.http.post<void>(this.url + '/ratings', rating, this.getHttpOptions()).pipe(
      catchError((err:any) => {
        console.log(err);
        return throwError(
          () =>
          new Error(
            'RecipeService.addRecipeRating(): error adding recipe rating'
          )
        );
